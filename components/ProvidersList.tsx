...
type Props = {
  ...
  /** optional layout: "grid" shows 3×N tiles, "list" shows vertical list */
  layout?: "grid" | "list";
};
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 space-y-2">
        {p.policy ? (
          <p className="small text-(--muted-foreground)">{p.policy}</p>
        ) : null}
        {p.caution ? <p className="small text-red-300">{p.caution}</p> : null}
...
export default function ProvidersList({
  ...
  if (layout === "grid") {
    return <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{cards}</div>;
  }
