...
export async function POST(req: NextRequest) {
  ...
  const EMAILJS_AUTOREPLY_TEMPLATE_ID = process.env.EMAILJS_AUTOREPLY_TEMPLATE_ID || '';
  ...
  if (!name || name.length < 2) return jsonError(400, 'BAD_NAME', 'Please provide your full name.');
  if (!email || !/^\S+@\S+\.\S+$/.test(email)) return jsonError(400, 'BAD_EMAIL', 'Please provide a valid email address.');
  if (!subject || subject.length < 3) return jsonError(400, 'BAD_SUBJECT', 'Please include a subject.');
  if (!message || message.length < 10) return jsonError(400, 'BAD_MESSAGE', 'Please include a longer message.');

  // Attachment ≤ 5MB
  if (attachment) {
    if (!attachment.filename || !attachment.content?.startsWith('data:')) {
      return jsonError(400, 'BAD_ATTACHMENT', 'Attachment must be a data URL with a filename.');
    }
    const { isBase64 } = parseDataUrlMeta(attachment.content);
    if (!isBase64) return jsonError(400, 'BAD_ATTACHMENT_ENCODING', 'Attachment must be base64-encoded.');
    const b64 = attachment.content.split(',')[1] || '';
    const bytes = Math.floor((b64.length * 3) / 4);
    if (bytes > 5 * 1024 * 1024) return jsonError(400, 'ATTACHMENT_TOO_LARGE', 'Attachment must be ≤ 5 MB.');
  }
  ...
  const page_url = referer && /^https?:\/\//i.test(referer) ? referer : `${originUrl.origin}/contact`;

  // reCAPTCHA (skip only for localhost + dev-bypass OR explicit 'dev-bypass' token)
  const shouldBypass = (DEV_BYPASS && isLocalHost(originUrl)) || token === 'dev-bypass';
  if (!shouldBypass) {
    if (!RECAPTCHA_SECRET) return jsonError(500, 'SERVER_MISCONFIG', 'reCAPTCHA is not configured on the server.');
    if (!token) return jsonError(400, 'MISSING_RECAPTCHA', 'reCAPTCHA token missing.');
    try {
  ...
  try {
    emailResp = await fetchWithTimeout('https://api.emailjs.com/api/v1.0/email/send', {
      method: 'POST',
      headers: {
  ...
  if (!emailResp.ok) {
    ...
    const code = emailResp.status === 401 ? 'EMAILJS_UNAUTHORIZED' : 'EMAILJS_ERROR';
